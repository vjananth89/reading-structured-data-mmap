...
/* code for converting to greyscale begins */
static void grey(void *bm){
...
unsigned long v =0;
char x;
for (i=0;i<pbminfo->height;i++){
for (j=0;j<pbminfo->width;j++){
x = (prow[v].blue+ prow[v].green + prow[v].red)/3;
    prow[v].blue = prow[v].green = prow[v].red = (char)x;
v++;
}}
}
